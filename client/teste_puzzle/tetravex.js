export default class Tetravex extends Phaser.Scene {
    ...
    init(data) {
        ...
        this.cnf = {
            ...
            main_space_x: null, main_space_y: null,
            main_space_wd: null, main_space_ht: null
        };
    ...
    create() {
        ...
        this.canvasTexture = this.textures.createCanvas('tetravex_canvas', width, height);
        ...
        this.add_action_btn(this.handle_btn_change_size.bind(this), "Change Size", 130);
        this.add_action_btn(this.handle_btn_solution.bind(this), "Show Solution", 150);
    ...
    resetConfig() {
        ...
        let horz_cell_sz = (cnv_wd - gutter_min * (vertical ? 2 : 3)) / (this.cells_x * (vertical ? 1 : 2));
        let vert_cell_sz = (cnv_ht - gutter_min * (vertical ? 3 : 2) - this.BTN_HT) / (this.cells_y * (vertical ? 2 : 1));
        this.cnf.cell_wd = this.cnf.cell_ht = Math.min(horz_cell_sz, vert_cell_sz);

        let gutter_x = (cnv_wd - this.cells_x * this.cnf.cell_wd * (vertical ? 1 : 2)) / (vertical ? 2 : 3);
        let gutter_y = (cnv_ht - this.cells_y * this.cnf.cell_ht * (vertical ? 2 : 1) - this.BTN_HT) / (vertical ? 3 : 2);
        ...
        if (vertical) {
            ...
            this.cnf.unplaced_pos.y = this.cnf.assembly_pos.y + this.cells_y * this.cnf.cell_ht + gutter_y;
        } else {
            this.cnf.unplaced_pos.x = this.cnf.assembly_pos.x + this.cells_x * this.cnf.cell_wd + gutter_x;
    ...
    hslToCss(h, s, l) { return `hsl(${h}, ${s * 100}%, ${l * 100}%)`; }
    ...
    draw_everything() {
        ...
        if (this.global_mode === "boards") {
            this.draw_winningness(ctx, this.cnf.assembly_pos.x, this.cnf.assembly_pos.y);
            for (let i = 0; i < 2; ++i) {
                this.draw_board(ctx, this.BOARDS[i].board, this.BOARDS[i].pos.x, this.BOARDS[i].pos.y);
            }
            this.draw_buttons(ctx);
            if (this.dragging_tile) {
                this.draw_tile(ctx, this.dragging_tile_x - this.cnf.cell_wd / 2 - 1, this.dragging_tile_y - this.cnf.cell_ht / 2 - 1, this.dragging_tile);
            }
    ...
    draw_board(ctx, board, bx, by) {
        ...
        for (let x = 0; x <= this.cells_x; ++x) {
            ...
            ctx.lineTo(bx + x * this.cnf.cell_wd, by + this.cells_y * this.cnf.cell_ht);
        }
        for (let y = 0; y <= this.cells_y; ++y) {
            ctx.moveTo(bx, by + y * this.cnf.cell_ht);
            ctx.lineTo(bx + this.cells_x * this.cnf.cell_wd, by + y * this.cnf.cell_ht);
        }
        ctx.strokeStyle = "#666";
        ctx.stroke();
        ctx.restore();
        ...
        for (let yi = 0; yi < this.cells_y; ++yi) {
            for (let xi = 0; xi < this.cells_x; ++xi) {
                this.draw_tile(ctx, bx + xi * this.cnf.cell_wd, by + yi * this.cnf.cell_ht, board[yi][xi]);
            }
        }
    }
    ...
    draw_tile(ctx, x, y, tile) {
        ...
        let left_x = x + 1, right_x = x + this.cnf.cell_wd - 1, mid_x = x + (this.cnf.cell_wd / 2);
        let top_y = y + 1, bottom_y = y + this.cnf.cell_ht - 1, mid_y = y + (this.cnf.cell_ht / 2);

        this.draw_triangle(ctx, tile.top, "top", left_x, top_y, right_x, top_y, mid_x, mid_y);
        this.draw_triangle(ctx, tile.right, "right", right_x, top_y, right_x, bottom_y, mid_x, mid_y);
        this.draw_triangle(ctx, tile.bottom, "bottom", right_x, bottom_y, left_x, bottom_y, mid_x, mid_y);
        this.draw_triangle(ctx, tile.left, "left", left_x, bottom_y, left_x, top_y, mid_x, mid_y);
        ctx.restore();
    }
    ...
    draw_triangle(ctx, num, which, x1, y1, x2, y2, x3, y3) {
        ...
        let midx12 = (x1 + x2) / 2, midy12 = (y1 + y2) / 2;
        let midx23 = (x2 + x3) / 2, midy23 = (y2 + y3) / 2;
        let midx31 = (x3 + x1) / 2, midy31 = (y3 + y1) / 2;
        let EDGE_SIZE = 0.1;
        let inx1 = x1 + (midx23 - x1) * EDGE_SIZE, iny1 = y1 + (midy23 - y1) * EDGE_SIZE;
        let inx2 = x2 + (midx31 - x2) * EDGE_SIZE, iny2 = y2 + (midy31 - y2) * EDGE_SIZE;
        let inx3 = x3 + (midx12 - x3) * EDGE_SIZE, iny3 = y3 + (midy12 - y3) * EDGE_SIZE;

        this.draw_triangle_edge(ctx, num, which, 1, x1, y1, x2, y2, inx2, iny2, inx1, iny1);
        this.draw_triangle_edge(ctx, num, which, 2, x2, y2, x3, y3, inx3, iny3, inx2, iny2);
        this.draw_triangle_edge(ctx, num, which, 3, x3, y3, x1, y1, inx1, iny1, inx3, iny3);
    ...
    draw_triangle_edge(ctx, num, which, side, x1, y1, x2, y2, x3, y3, x4, y4) {
        ...
        if ((which === "top" && side === 2) || (which === "right" && side === 1) || (which === "bottom" && side !== 2) || (which === "left" && side !== 1)) {
    ...
    draw_buttons(ctx) {
        ...
        for (let i = 0; i < this.cur_btns_all.length; ++i) {
            ...
            ctx.fillStyle = "#fff";
            ctx.fillText(btn.label, btn.x + btn.width / 2, btn.y + btn.height / 2, btn.width - 20);
        }
        ctx.restore();
    }
    ...
    update_winingness() {
        ...
        for (let xi = 0; xi < this.cells_x; ++xi) {
            for (let yi = 0; yi < this.cells_y; ++yi) {
                ...
                else if ((right && tile.right !== right.left) || (below && tile.bottom !== below.top)) {
    ...
    create_starting_board() {
        ...
        for (let i = 0; i < num_tiles - 1; ++i) {
            let xi = i % this.cells_x; let yi = Math.floor(i / this.cells_x);
            let j = Math.floor(Math.random() * (num_tiles - i)) + i;
            let xj = j % this.cells_x; let yj = Math.floor(j / this.cells_x);
    ...
    find_pos_in_boards(pos) {
        for (let i = 0; i < 2; ++i) {
            let bx = this.BOARDS[i].pos.x; let by = this.BOARDS[i].pos.y;
            let x = pos.x - bx; let y = pos.y - by;
            if (x >= 0 && y >= 0) {
                let xi = Math.floor(x / this.cnf.cell_wd);
                let yi = Math.floor(y / this.cnf.cell_ht);
                if (xi < this.cells_x && yi < this.cells_y) {
                    return { board: this.BOARDS[i].board, xi: xi, yi: yi, tile: this.BOARDS[i].board[yi][xi] };
                }
            }
        }
        return null;
    ...
    find_pos_in_buttons(pos) {
        for (let i = 0; i < this.cur_btns_all.length; ++i) {
            let btn = this.cur_btns_all[i];
            if (pos.x >= btn.x && pos.x <= (btn.x + btn.width) && pos.y >= btn.y && pos.y <= (btn.y + btn.height)) return btn;
        }
        return null;
    }
    ...
    handleMouseUp(pointer) {
        ...
        if (this.pressing_button) {
            ...
            let match = this.find_pos_in_boards(pos);
            if (match) {
                this.dragging_tile_board[this.dragging_tile_yi][this.dragging_tile_xi] = match.tile;
                this.dragging_tile_board = match.board;
                this.dragging_tile_xi = match.xi;
                this.dragging_tile_yi = match.yi;
            }
            this.dragging_tile_board[this.dragging_tile_yi][this.dragging_tile_xi] = this.dragging_tile;
            this.dragging_tile = null;
            this.update_winingness();
            this.save_boards();
        }
    ...
    handle_btn_change_size_cancel() { this.pop_global_mode(); }
    ...
    handle_btn_change_size() {
        ...
        this.add_action_btn(this.handle_btn_change_size_cancel.bind(this), "Cancel", 100);

        let btn_wd = this.cnf.main_space_wd / 3;
        let btn_ht = this.cnf.main_space_ht / 3;
        const tamanhosPermitidos = [{ w: 3, h: 2 }, { w: 2, h: 3 }, { w: 2, h: 2 }];

        for (let i = 0; i < tamanhosPermitidos.length; ++i) {
            let wd = tamanhosPermitidos[i].w; let ht = tamanhosPermitidos[i].h;
            let x = this.cnf.main_space_x + btn_wd * i; let y = this.cnf.main_space_y + btn_ht;

            this.cur_btns_all.push({
                x: x + 10, y: y, width: btn_wd - 20, height: btn_ht - 10,
                label: `${wd}×${ht}`,
                handler: () => {
                    this.cells_x = wd; this.cells_y = ht;
                    this.create_solution(); this.create_starting_board();
                    this.save_boards(); this.pop_global_mode();
                },
            });
        }
        this.resetConfig();
    ...
    handle_btn_solution() {
        let targetYi = -1, targetXi = -1;
        outerLoop:
        for (let yi = 0; yi < this.cells_y; ++yi) {
            for (let xi = 0; xi < this.cells_x; ++xi) {
                if (this.ASSEMBLY[yi][xi] !== this.SOLUTION[yi][xi]) {
                    targetYi = yi; targetXi = xi; break outerLoop;
                }
            }
        }
        ...
        let foundBoard = null, foundYi = -1, foundXi = -1;

        for (let yi = 0; yi < this.cells_y; ++yi) {
            for (let xi = 0; xi < this.cells_x; ++xi) {
                if (this.UNPLACED[yi][xi] === correctTile) {
                    foundBoard = this.UNPLACED; foundYi = yi; foundXi = xi; break;
                }
            }
        }
        if (!foundBoard) {
            for (let yi = 0; yi < this.cells_y; ++yi) {
                for (let xi = 0; xi < this.cells_x; ++xi) {
                    if (this.ASSEMBLY[yi][xi] === correctTile) {
                        foundBoard = this.ASSEMBLY; foundYi = yi; foundXi = xi; break;
                    }
                }
            }
        }
        ...
        for (let bi = 0; bi < this.BOARDS_TO_SAVE.length; ++bi) {
            ...
            for (let yi = 0; yi < this.cells_y; ++yi) {
                for (let xi = 0; xi < this.cells_x; ++xi) {
                    ...
                    if (!tile) { tiles.push("."); continue; }
                    for (let si = 0; si < this.TILES_SIDES.length; ++si) tiles.push(tile[this.TILES_SIDES[si]]);
                }
            }
        }
    ...
    load_boards() {
        let match = document.cookie.match(/(?:^|;\s*)tetravex_boards=([.,0-9]+)(?:;|$)/);
        if (!match) return false;
        match = match[1].match(/^(\d+),(\d+),([.0-9]+)$/);
        if (!match) return false;

        let cx = +match[1], cy = +match[2];
        if ((cx === 3 && cy === 2) || (cx === 2 && cy === 3) || (cx === 2 && cy === 2)) {
            this.cells_x = cx; this.cells_y = cy;
        } else {
            this.cells_x = 3; this.cells_y = 2; return false;
        }

        let tiles = match[3]; let pos = 0;
        for (let bi = 0; bi < this.BOARDS_TO_SAVE.length; ++bi) {
            let board = this.BOARDS_TO_SAVE[bi]; board.length = 0;
            for (let yi = 0; yi < this.cells_y; ++yi) {
                let row = []; board.push(row);
                for (let xi = 0; xi < this.cells_x; ++xi) {
                    let c = tiles[pos];
                    if (c === ".") { row.push(null); ++pos; continue; }
                    let tile = {};
                    for (let si = 0; si < this.TILES_SIDES.length; ++si) tile[this.TILES_SIDES[si]] = +tiles[pos + si];
                    row.push(tile); pos += 4;
                }
            }
        }
