export default class Genius extends Phaser.Scene {
    ...
    create() {
        ...
        this.textoStatus = this.add.text(640, 100, "MEMORIZE A SEQUÊNCIA", { 
        ...
        const posicoes = [
            { x: 530, y: 250 }, { x: 750, y: 250 },
            { x: 530, y: 470 }, { x: 750, y: 470 }  
        ];

        for (let i = 0; i < 4; i++) {
            let btn = this.add.rectangle(posicoes[i].x, posicoes[i].y, 200, 200, this.cores[i].normal)
                .setInteractive({ useHandCursor: true })
                .setStrokeStyle(4, 0x333333);

            let textoSimbolo = this.add.text(posicoes[i].x, posicoes[i].y, simbolos[i], {
                fontSize: '70px',
                fill: '#ffffff', 
                fontStyle: 'bold'
            }).setOrigin(0.5);
